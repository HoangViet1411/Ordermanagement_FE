...
export class OrderService {
  ...
  async updateOrder(id: number, orderData: UpdateOrderDto): Promise<OrderResponse | null> {
    ...
    if (orderData.items !== undefined) {
      ...
      // Xử lý items: update items còn lại, delete items không còn, create items mới
      const now = new Date();
      
      // 1. Update items còn lại (giữ ID cũ)
      for (const [productId, existingItem] of existingItemsMap) {
        const newItem = newItemsMap.get(productId);
        if (newItem) {
          // Item còn lại - update quantity và unit_price nếu có thay đổi
          const product = productMap.get(productId)!;
          ...
          if (needsUpdate) {
            await existingItem.update({
              ...
              unitPrice,
              updatedAt: now,
              // lineTotal sẽ được trigger tự động tính khi update quantity hoặc unitPrice
            });
          }
        } else {
          // Item không còn trong danh sách mới - delete
          await existingItem.destroy();
        }
      }

      // 2. Create items mới (chưa có trong items cũ)
      ...
      // Bulk create items mới - trigger sẽ tự động tính line_total và total_amount
      if (itemsToCreate.length > 0) {
        await OrderDetail.bulkCreate(itemsToCreate);
      }

      // totalAmount sẽ được trigger tự động tính khi update/create/delete items
    }
    ...
    // Nếu có update items, cần reload với include items để lấy items mới với ID mới
    const reloadOptions: {
      attributes: string[];
      include?: any[];
    } = {
    ...
    // Nếu có update items, include items để lấy items mới với ID mới
    if (orderData.items !== undefined) {
