...
export const createUserSchema = Joi.object({
  cognito_user_id: Joi.string().max(255).optional().allow(null, '').messages({
    'string.base': 'cognito_user_id must be a string',
    'string.max': 'cognito_user_id must be less than 255 characters',
  }),
...
// Schema for Create Profile (required first_name và last_name)
export const profileCreateSchema = Joi.object({
  ...
  birth_date: createBirthDateSchema(false),
  gender: createGenderSchema(false),
  // role_ids đã bị bỏ - user không được tự chọn role
});

// Schema for Update Profile (optional tất cả fields - cho phép update một phần)
